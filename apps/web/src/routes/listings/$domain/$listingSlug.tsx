...
function PreviewTableText({
  ...
  return (
    <span className={`block truncate whitespace-nowrap ${maxWidthClassName}`} title={value}>
      {value}
    </span>
  )
}
      ...
      <div className="space-y-6">
                  ...
                  <TR>
                    {previewQuery.data.data.columns.map((column) => (
                      <TH key={column} className="align-top">
                        <PreviewTableText value={column} maxWidthClassName="max-w-40" />
                      </TH>
                    ))}
                  </TR>
                </THead>
                <TBody>
                  {previewQuery.data.data.rows.slice(0, 5).map((row, index) => (
                    <TR key={`${listing.dataset.id}:preview:${index}`}>
                      {previewQuery.data?.data.columns.map((column) => {
                        const cellValue = formatPreviewValue(row[column])

                        return (
                          <TD
                            key={`${listing.dataset.id}:${index}:${column}`}
                            className="align-top"
                          >
                            <PreviewTableText value={cellValue} maxWidthClassName="max-w-[20rem]" />
                          </TD>
                        )
                      })}
                    </TR>
        ...
        <Card className="space-y-3">
          <div className="space-y-1">
            <h2 className="text-lg font-semibold">Resources & analysis quick start</h2>
            <p className="text-sm text-[var(--muted-foreground)]">
              {listing.domain === 'github'
                ? 'SQL로 활동 후보를 찾고 documentation/notebook 리소스로 enrich evidence까지 내려가 보세요.'
                : 'SQL, API, documentation 리소스로 이 dataset을 바로 탐색해 보세요.'}
            </p>
          </div>
