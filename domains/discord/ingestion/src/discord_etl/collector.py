...
def collect_channel(
    ...
    *,
    ...
    - watermark는 D1에서 읽어 증분 수집 기준으로 사용
    - watermark 갱신은 upload 커맨드에서 처리
    - scan_cursor는 로컬 .cursor 파일로 중단 복구용으로 저장
    """
    ...
    try:
        ...
        if scan_cursor:
            ...
            logger.info(
                ...
                scan_cursor,
                channel.name,
                extra={"event_code": "SCAN_RESUME", "channel_id": channel.id},
            )
        ...
        while True:
            ...
            if not raw_messages:
                logger.info(
                    ...
                    channel.name,
                    extra={"event_code": "HISTORY_END", "channel_id": channel.id},
                )
                break
            ...
            for raw in raw_messages:
                try:
                    ...
                    logger.warning("Parse error: %s (raw_id=%s)", e, raw.get("id", "?"))
                    continue
            ...
            if all(int(raw["id"]) <= int(watermark_id) for raw in raw_messages):
                reached_watermark = True
                logger.info(
                    "Reached watermark (channel=%s, watermark=%s)",
                    channel.name,
                    watermark_id,
                    extra={"event_code": "WATERMARK_REACHED", "channel_id": channel.id},
                )
            ...
            if chunk_page_count >= config.discord.chunk_pages or reached_watermark:
                ...
                if not dry_run and not reached_watermark and before is not None:
                    if output_dir is not None:
        ...
        if not dry_run and output_dir is None and int(max_collected_id) > int(watermark_id):
            ...
            logger.info(
                ...
                max_collected_id,
                channel.name,
                extra={"event_code": "WATERMARK_UPDATED", "channel_id": channel.id},
            )
        ...
        logger.error(
            ...
            channel.name,
            e,
            extra={"event_code": "COLLECT_ERROR", "channel_id": channel.id},
        )
    except Exception as e:
        stats.error = str(e)
        logger.error(
...
def _flush_chunk(
    ...
    *,
    ...
    new_messages = [msg for msg in buffer if int(msg.id) > int(watermark_id)]
    ...
    if output_dir is not None:
        ...
        raw_to_write = [r for r in (raw_messages or []) if r.get("id") in new_ids]
